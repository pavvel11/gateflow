...
PRODUCT_ID=$(docker exec supabase_db_sellf psql -U postgres -d postgres -t -c "SELECT id FROM products WHERE is_active = true LIMIT 1;" | tr -d ' ')

if [ -z "$PRODUCT_ID" ]; then
  echo "❌ Error: No active products found in database!"
  echo "Creating a test product first..."
  PRODUCT_ID=$(docker exec supabase_db_sellf psql -U postgres -d postgres -t -c "
