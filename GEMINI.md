...
We use `pm2` to manage local processes during development.
...
# Type generation
supabase gen types typescript --local > admin-panel/src/types/database.ts
```
...
- **PM2 for Local Execution**: Use `pm2` to manage and run the application services locally.
...
5. **Verify (Standards):** VERY IMPORTANT: After making ANY code changes, execute the project-specific build, linting and type-checking commands (e.g., 'tsc', 'npm run lint', 'ruff check .', 'npm run build') to ensure code quality and adherence to standards. 
   - **Compilations Check:** ALWAYS run `npm run build` (or equivalent) to verify there are no compilation errors before finishing the task.
   - **Database Changes:** If you modified the database schema (migrations), you MUST regenerate TypeScript types (`supabase gen types typescript --local > admin-panel/src/types/database.ts` or similar) and run a build check (`npm run build`) to ensure type safety is maintained.
