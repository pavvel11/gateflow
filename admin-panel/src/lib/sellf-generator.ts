 ...
 */
export class SellfGenerator {
  /**
   * Generate the complete sellf script with injected configuration
   */
  static async generateScript(config: SellfConfig): Promise<GeneratedScript> {
   ...
   */
  private static async buildScript(config: SellfConfig): Promise<string> {
    const configSection = this.generateConfigSection(config);
    const coreScript = await this.getCoreScript();

    // Simple template - no wrapper block needed (sellf.js handles its own initialization)
...
${this.generateFooter(config)}`;
    ...
    template = template.replace(/\{\{SELLF_VERSION\}\}/g, config.version);
...
const GATEKEEPER_CONFIG = Object.freeze({
  ...
  SELLF_VERSION: '${safeConfig.SELLF_VERSION}',
   ...
   */
  private static generateHeader(config: SellfConfig): string {
    return `/*!
 * Sellf.js v${config.version}
 * Generated by Sellf Admin Panel
 * 
 ...
 * @website https://sellf.app
 * @support support@sellf.app
 */
...
'use strict';`;
  ...
  /**
   * Get the core script content (the actual sellf logic)
   */
  private static async getCoreScript(): Promise<string> {
    ...
    try {
      const sellfPath = join(process.cwd(), '../sellf.js');
      let content = readFileSync(sellfPath, 'utf-8');
      ...
      console.warn('sellf.js not found, using fallback:', error);
  ...
  private static getInlineCoreScript(): string {
    return `
// Core Sellf functionality will be loaded here
// This is a fallback when sellf.js is not found
...
window.sellf = {
  ...
  init: function() {
    console.log('Sellf initialized in fallback mode');
  },
...
`;
   ...
   */
  private static generateFooter(config: SellfConfig): string {
    return `
...
if (typeof GATEKEEPER_CONFIG !== 'undefined') {
  ...
  if (GATEKEEPER_CONFIG.ENVIRONMENT === 'development') {
    console.log('[Sellf] Config loaded:', GATEKEEPER_CONFIG.SELLF_VERSION);
  }
}
`;
  }
   ...
   */
  private static getCacheKey(config: SellfConfig): string {
   ...
   */
  private static generateBuildHash(config: SellfConfig): string {
