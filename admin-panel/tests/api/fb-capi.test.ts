...
describe('POST /api/tracking/fb-capi', () => {
  ...
  describe('Rate limiting', () => {
    ...
    it('should return 429 after exceeding rate limit (requires RATE_LIMIT_TEST_MODE=true)', async () => {
      ...
      // If rate limiting is skipped (dev mode), all will be 400/500 depending on config
      ...
      if (!has429) {
        ...
        console.log(
          '[fb-capi test] Rate limiting skipped (RATE_LIMIT_TEST_MODE not enabled on dev server)'
        );
      } else {
