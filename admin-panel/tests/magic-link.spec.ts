...
test.describe('Magic Link Authentication (Mailpit)', () => {
  ...
  test.skip('session should expire after logout (mocked replay protection)', async ({ page }) => {
    // SKIPPED: Same issue with browser session mocking
    // Mock replay attack by logging out and trying to reuse session
    const supabaseAdmin = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
      process.env.SUPABASE_SERVICE_ROLE_KEY!
    );

    const mockEmail = `replay-${Date.now()}@example.com`;
    ...
    // 4. Try to access dashboard again (should be redirected to login)
    await page.goto('/dashboard');
    await page.waitForTimeout(2000);

    // Should NOT be on dashboard (session expired)
    const currentUrl = page.url();
